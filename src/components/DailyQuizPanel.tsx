...
const RANK_STYLES = [
  "text-yellow-400 font-black",   // 1st
  "text-slate-300 font-black",    // 2nd
  "text-orange-400 font-black",   // 3rd
];
...
export default function DailyQuizPanel() {
  ...
  const loadLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${supabaseUrl}/functions/v1/quiz-leaderboard?date=${today}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    if (activeTab === "leaderboard") {
      loadLeaderboard();
    } else if (isAdmin && !quiz) {
      loadQuiz();
      checkPriorResult();
    }
  }, [open, activeTab, isAdmin, quiz, loadQuiz, checkPriorResult, loadLeaderboard]);
            ...
            >
              ...
              <div className="flex border-b border-border/50 shrink-0">
                {(["quiz", "leaderboard"] as const).map((tab) => (
                  ...
                  >
                    {tab === "quiz" ? (
                      <><CalendarDays className="w-3 h-3" /> Quiz</>
                    ) : (
                  ...
                  <div className="p-4">
                    {lbLoading ? (
                      <div className="flex flex-col items-center justify-center py-16 gap-3">
                        ...
                        <p className="text-xs text-muted-foreground">Loading today's scores…</p>
                      </div>
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">No scores yet today</p>
                      ...
                      <div className="space-y-1">
                        ...
                        {leaderboard.slice(0, 3).length > 0 && (
                          <div className="flex items-end justify-center gap-2 mb-5 pt-2 pb-1">
                            {/* 2nd */}
                            {leaderboard[1] && (
                              <motion.div
                              ...
                              >
                                <div className="w-9 h-9 rounded-full bg-muted border-2 border-slate-400/50 flex items-center justify-center text-sm font-black text-muted-foreground overflow-hidden">
                                  {leaderboard[1].avatar_url
                                    ? <img src={leaderboard[1].avatar_url} className="w-full h-full object-cover" />
                                    : leaderboard[1].name.charAt(0).toUpperCase()}
                                </div>
                                <span className="text-[10px] text-muted-foreground font-medium max-w-[56px] truncate text-center">
                                  {leaderboard[1].name}
                                </span>
                                <div className="w-12 h-8 bg-muted/60 border border-border rounded-t-lg flex items-center justify-center">
                            ...
                            >
                              ...
                              <div className="w-11 h-11 rounded-full bg-primary/20 border-2 border-primary/60 flex items-center justify-center text-sm font-black text-primary overflow-hidden">
                                {leaderboard[0].avatar_url
                                  ? <img src={leaderboard[0].avatar_url} className="w-full h-full object-cover" />
                                  : leaderboard[0].name.charAt(0).toUpperCase()}
                              </div>
                              <span className="text-[10px] text-foreground font-semibold max-w-[64px] truncate text-center">
                                {leaderboard[0].name}
                              </span>
                              <div className="w-12 h-11 bg-primary/10 border border-primary/30 rounded-t-lg flex items-center justify-center">
                            ...
                            {leaderboard[2] && (
                              ...
                              >
                                <div className="w-9 h-9 rounded-full bg-muted border-2 border-orange-400/50 flex items-center justify-center text-sm font-black text-muted-foreground overflow-hidden">
                                  {leaderboard[2].avatar_url
                                    ? <img src={leaderboard[2].avatar_url} className="w-full h-full object-cover" />
                                    : leaderboard[2].name.charAt(0).toUpperCase()}
                                </div>
                                <span className="text-[10px] text-muted-foreground font-medium max-w-[56px] truncate text-center">
                                  {leaderboard[2].name}
                                </span>
                                <div className="w-12 h-6 bg-muted/60 border border-border rounded-t-lg flex items-center justify-center">
                        ...
                        <div className="space-y-1">
                          {leaderboard.map((entry, i) => {
                            ...
                            return (
                              ...
                              >
                                {/* Rank */}
                                <span className={cn("w-5 text-center shrink-0 text-xs", RANK_STYLES[i] ?? "text-muted-foreground font-medium")}>
                                  {entry.rank}
                                </span>

                                {/* Avatar */}
                                <div className="w-7 h-7 rounded-full bg-muted border border-border flex items-center justify-center text-xs font-bold text-muted-foreground overflow-hidden shrink-0">
                                  {entry.avatar_url
                                ...
                                {/* Name */}
                                <div className="flex-1 min-w-0">
                                  <p className={cn("font-semibold truncate", isMe ? "text-primary" : "text-foreground")}>
                                    {entry.name}{isMe && <span className="text-[9px] ml-1 text-primary/70">(you)</span>}
                                  </p>
                                ...
                                <div className="text-right shrink-0">
                                  <p className={cn("font-black", entry.pct >= 80 ? "text-primary" : "text-foreground")}>
                                    {entry.score}/{entry.total}
                                  </p>
                                  <p className="text-muted-foreground">{entry.pct}%</p>
                                </div>
                              </motion.div>
                            );
                          })}
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-2 justify-center">
                          {["Architecture", "Clarity", "DeFi", "Security", "sBTC"].map((t) => (
                            <span key={t} className="text-[10px] px-2 py-1 rounded-md bg-muted/50 border border-border/50 text-muted-foreground">
                              {t}
                            </span>
                          ))}
                        </div>
                        ...
                        {!loading && !generating && alreadyDone && !finished && (
                          ...
                          >
                            ...
                            <div className="flex gap-2 justify-center">
                              ...
                              <Button variant="outline" size="sm" onClick={() => setActiveTab("leaderboard")} className="gap-2">
                        ...
                        {!loading && !generating && finished && quiz && (
                          ...
                          >
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.1 }}>
                              <Trophy className={cn("w-14 h-14 mx-auto", score / quiz.questions.length >= 0.8 ? "text-primary" : "text-muted-foreground")} />
                            </motion.div>
                            <div>
                              <p className="text-4xl font-black text-primary">{score}/{quiz.questions.length}</p>
                            ...
                            {currentStreak > 0 && (
                              ...
                              >
                                ...
                                <div className="text-left">
                                  <p className="text-xs font-bold text-foreground">{currentStreak} day streak!</p>
                                  <p className="text-[10px] text-muted-foreground">Best: {longestStreak} day{longestStreak !== 1 ? "s" : ""}</p>
                                </div>
                              </motion.div>
                            )}
                            {/* Per-question review */}
                            <div className="space-y-2 text-left max-h-52 overflow-y-auto pr-1">
                              {quiz.questions.map((q, i) => (
                                ...
                                >
                                  ...
                                  <p className="text-muted-foreground leading-relaxed line-clamp-2">{q.question}</p>
                                </div>
                              ))}
                            </div>
                            <div className="flex gap-2 justify-center">
                              ...
                              <Button size="sm" onClick={() => { setActiveTab("leaderboard"); loadLeaderboard(); }} className="gap-2">
                                <Trophy className="w-3.5 h-3.5" />
                                Leaderboard
                              </Button>
                            </div>
                        ...
                        {!loading && !generating && quiz && !finished && !alreadyDone && q && (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-base">{TOPIC_EMOJI[q.topic?.toLowerCase()] ?? "🧠"}</span>
                            ...
                            <div className="flex items-center justify-between pt-1">
                              ...
                              {!revealed ? (
                                <Button size="sm" onClick={handleSubmit} disabled={selected === null} className="h-8 px-5 text-xs">
                                  Submit
                                </Button>
                              ) : (
                                <Button size="sm" onClick={handleNext} className="h-8 px-5 text-xs gap-1">
                                  {current < quiz.questions.length - 1
                                    ? <><span>Next</span><ChevronRight className="w-3.5 h-3.5" /></>
                                    : <><span>Finish</span><Trophy className="w-3.5 h-3.5" /></>}
                                </Button>
                              )}
                            </div>
                          </motion.div>
